...
//                if (true) {
    ...
    private suspend fun sendReminder(
        ...
        try {
            ...
            val builder = NotificationCompat.Builder(context, "reminder")
                .setSmallIcon(com.example.ezblue.R.mipmap.ezbluelogo)
    ...
    private suspend fun performReminderTaskCheck(beacon: Beacon, context: Context): Boolean {
        ...
        return withContext(Dispatchers.IO) { // Making this function a return with context suspend function because I need to wait for the result before continuing with the task
            ...
            val lastLog = activityLogs.filter { it.status == LogResults.SUCCESS }
                .lastOrNull() //filtering to get the most recent log when it was a success
            if (lastLog == null) {
            ...
            val extractedLogDate = SimpleDateFormat(
                "dd-MM-yyyy HH:mm:ss",
                Locale.getDefault()
            ).parse(lastLog.timestamp) // Extracting the date from the logs
            ...
            Log.d(
                "TaskViewModel",
                "Last log time: $extractedLogDate, Next allowed time: $nextAllowedTime, Now: $now"
            )
